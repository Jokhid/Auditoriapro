...
function collectMetrics() {
  ...
  const benefits: Benefit[] = [
    { label: 'Baja laboral', ...benefitValue(base * 0.75) },
    { label: 'Invalidez absoluta', ...benefitValue(base) },
    { label: 'Invalidez profesional', ...benefitValue(age >= 55 ? base * 0.75 : base * 0.55) },
    { label: 'Viudedad', ...benefitValue(married ? base * 0.52 : 0) },
    { label: 'Orfandad', ...benefitValue(base * 0.2 * children) },
    { label: 'Jubilación', ...benefitValue(retirementPension) },
  ];
...
function clientFields(m: Metrics): Field[] {
  return [
    ...
    { label: 'Gasto mensual total utilizado en la app', value: moneyMonth(m.expenses) },
...
function rows(doc: jsPDF, state: PageState, fields: Field[], columns = 1) {
  ...
  fields.forEach((field, index) => {
    const col = columns === 1 ? 0 : index % 2; if (columns === 1 || col === 0) ensure(doc, state, rowH + 4);
...
function metricStrip(doc: jsPDF, state: PageState, metrics: Field[]) {
  ensure(doc, state, 30); const cardW = (W - 10) / 3;
  metrics.forEach((metric, i) => {
    const x = M + i * (cardW + 5); const y = state.y;
    doc.setFillColor(255, 255, 255); doc.setDrawColor(...GOLD); doc.roundedRect(x, y, cardW, 24, 3, 3, 'FD');
    doc.setFont('Helvetica', 'bold'); doc.setFontSize(6.5); doc.setTextColor(...MUTED); doc.text(metric.label, x + 4, y + 7);
    doc.setFont('Helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...SLATE); doc.text(doc.splitTextToSize(metric.value, cardW - 8).slice(0, 1), x + 4, y + 17);
  });
  state.y += 31;
}
function drawObjectivesTable(doc: jsPDF, state: PageState, projects: ProjectRow[]) {
  heading(doc, state, 'Objetivos a medio y largo plazo');
  paragraph(doc, state, 'Los objetivos se ordenan por proyecto para comprobar si el esfuerzo mensual requerido encaja con la capacidad de ahorro real. Se excluye el campo de objetivo personalizable y solo se muestran los proyectos añadidos.'); rule(doc, state);
  ensure(doc, state, 20); const widths = [58, 32, 22, 34, 30]; const headers = ['Proyecto', 'Importe', 'Años', 'Ahorro/mes', 'Estado']; let x = M;
  doc.setFillColor(...BLACK); doc.roundedRect(M, state.y, W, 10, 2, 2, 'F'); doc.setFont('Helvetica', 'bold'); doc.setFontSize(7); doc.setTextColor(255, 255, 255);
  headers.forEach((h, i) => { doc.text(h, x + 3, state.y + 6.5); x += widths[i]; }); state.y += 10;
  if (!projects.length) projects = [{ name: 'Sin proyectos añadidos', target: 0, years: 0, monthly: 0, status: 'No indicado' }];
  projects.forEach((project, rowIndex) => {
    ensure(doc, state, 10); x = M; doc.setFillColor(rowIndex % 2 ? 255 : 248, rowIndex % 2 ? 255 : 250, rowIndex % 2 ? 255 : 252); doc.setDrawColor(...BORDER); doc.rect(M, state.y, W, 10, 'FD');
    const values = [project.name, money(project.target), `${project.years}`, moneyMonth(project.monthly), project.status];
    values.forEach((value, i) => { doc.setFont('Helvetica', i === 4 ? 'bold' : 'normal'); doc.setFontSize(6.8); doc.setTextColor(i === 4 && project.status === 'No viable' ? RED[0] : i === 4 && project.status === 'Ajustado' ? ORANGE[0] : i === 4 ? GREEN[0] : SLATE[0], i === 4 && project.status === 'No viable' ? RED[1] : i === 4 && project.status === 'Ajustado' ? ORANGE[1] : i === 4 ? GREEN[1] : SLATE[1], i === 4 && project.status === 'No viable' ? RED[2] : i === 4 && project.status === 'Ajustado' ? ORANGE[2] : i === 4 ? GREEN[2] : SLATE[2]); doc.text(doc.splitTextToSize(value, widths[i] - 5).slice(0, 1), x + 3, state.y + 6.5); x += widths[i]; });
    state.y += 10;
  });
  state.y += 8;
}
...
function axis(doc: jsPDF, x: number, y: number, w: number, h: number, max: number, xLabels: string[], percentAxis = false) {
  ...
  [0, .25, .5, .75, 1].forEach((r) => { const yy = y + h - r * h; doc.line(x, yy, x + w, yy); doc.text(percentAxis ? `${Math.round(r * 100)}%` : shortMoney(max * r), x - 4, yy + 2, { align: 'right' }); });
...
function drawBenefitChart(doc: jsPDF, state: PageState, m: Metrics) {
  heading(doc, state, 'Comparativa gráfica de prestaciones frente a gastos', 11); paragraph(doc, state, `Cada barra representa la prestación pública estimada y la línea roja representa los gastos mensuales requeridos (${moneyMonth(m.expenses)}).`);
  ensure(doc, state, 116); const x = 43, top = state.y + 4, w = 140, h = 82; const max = Math.max(m.expenses, ...m.benefits.map((b) => b.value), 1); axis(doc, x, top, w, h, max, m.benefits.map((b) => b.label));
  const slot = w / m.benefits.length; const barW = slot * 0.48;
  m.benefits.forEach((b, i) => { const bh = (b.value / max) * h; const bx = x + i * slot + slot * 0.25; doc.setFillColor(...GOLD); doc.roundedRect(bx, top + h - bh, barW, bh, 1, 1, 'F'); doc.setFont('Helvetica', 'bold'); doc.setFontSize(6.2); doc.setTextColor(...SLATE); doc.text(shortMoney(b.value), bx + barW / 2, top + h - bh - 2, { align: 'center' }); });
  const gy = top + h - (m.expenses / max) * h; doc.setDrawColor(...RED); doc.setLineWidth(0.9); doc.line(x, gy, x + w, gy); doc.setFont('Helvetica', 'bold'); doc.setFontSize(7); doc.setTextColor(...RED); doc.text(`Gastos: ${shortMoney(m.expenses)}`, x + w, gy - 2, { align: 'right' }); state.y = top + h + 18;
}
function drawRetirementChart(doc: jsPDF, state: PageState, m: Metrics) {
  heading(doc, state, 'Gráfico de proyección de jubilación', 11); paragraph(doc, state, 'El gráfico refleja la proyección patrimonial construida con los datos de la app: patrimonio actual, rentabilidad, ahorro sistemático, rentas inmobiliarias e inversiones inmobiliarias.');
  ensure(doc, state, 120); const x = 43, top = state.y + 4, w = 140, h = 84; const max = Math.max(m.projectedTotal, m.targetCapital, m.bank, 1); axis(doc, x, top, w, h, max, [`${m.age || 0} años`, `${Math.round((m.age + 67) / 2)} años`, '67 años']);
  const points = Array.from({ length: 8 }, (_, i) => { const r = i / 7; const val = m.bank + m.projectedInvested * r + m.projectedSaving * r + m.projectedRents * r + m.realEstateInvestments * r; return { px: x + w * r, py: top + h - (val / max) * h, val }; });
  doc.setDrawColor(...GOLD); doc.setLineWidth(1.3); points.forEach((p, i) => { if (i) doc.line(points[i - 1].px, points[i - 1].py, p.px, p.py); doc.setFillColor(...GOLD); doc.circle(p.px, p.py, 1.5, 'F'); });
  const targetY = top + h - (m.targetCapital / max) * h; doc.setDrawColor(...RED); doc.setLineDashPattern([3, 2], 0); doc.line(x, targetY, x + w, targetY); doc.setLineDashPattern([], 0); doc.setTextColor(...RED); doc.setFont('Helvetica', 'bold'); doc.setFontSize(7); doc.text(`Capital objetivo: ${shortMoney(m.targetCapital)}`, x + w, targetY - 2, { align: 'right' }); doc.setTextColor(...GOLD); doc.text(`Proyección: ${shortMoney(m.projectedTotal)}`, x + w, points[points.length - 1].py - 3, { align: 'right' }); state.y = top + h + 18;
}
function drawRadarChart(doc: jsPDF, state: PageState, m: Metrics) {
  heading(doc, state, 'Gráfico de seguridad y vulnerabilidad', 11); paragraph(doc, state, 'El gráfico radial usa las mismas reglas de puntuación de la aplicación para representar la seguridad en cada área crítica.');
  ensure(doc, state, 118); const scores = appScores(m); const cx = 105, cy = state.y + 54, radius = 40; const sides = scores.length;
  ...
  doc.setDrawColor(...GOLD); doc.setLineWidth(1.1); poly.forEach((p, i) => { const n = poly[(i + 1) % poly.length]; doc.line(p[0], p[1], n[0], n[1]); doc.setFillColor(...GOLD); doc.circle(p[0], p[1], 1.5, 'F'); }); state.y += 112;
}
function diagnosisRows(m: Metrics): Field[] {
  const sev = (gap: number, score = 10) => gap > 600 || score <= 3 ? 'Grave' : gap > 0 || score <= 6 ? 'Moderada' : 'Leve'; const scores = appScores(m).reduce<Record<string, number>>((acc, item) => { acc[item.label] = item.score; return acc; }, {});
  return [
    { label: `Baja laboral - ${sev(m.benefits[0].gap, scores.Baja)}`, value: m.benefits[0].gap > 0 ? `Brecha de ${moneyMonth(m.benefits[0].gap)}. Recomendación: complementar la prestación pública con cobertura privada temporal.` : 'La prestación estimada cubre el gasto mensual requerido.' },
    { label: `Invalidez profesional - ${sev(m.benefits[2].gap)}`, value: m.benefits[2].gap > 0 ? `Brecha de ${moneyMonth(m.benefits[2].gap)}. Recomendación: revisar capitales de invalidez y amortización de deuda.` : 'No se detecta brecha mensual relevante frente al gasto declarado.' },
    { label: `Protección familiar - ${sev(Math.max(m.benefits[3].gap, m.benefits[4].gap), scores.Familia)}`, value: `Viudedad estimada: ${moneyMonth(m.benefits[3].value)}. Orfandad estimada: ${moneyMonth(m.benefits[4].value)}. Recomendación: definir capital familiar objetivo.` },
    ...
    { label: `Jubilación - ${sev(m.retirementGap)}`, value: `Brecha mensual ajustada: ${moneyMonth(m.retirementGap)}. Capital objetivo: ${money(m.targetCapital)}. Ahorro recomendado: ${moneyMonth(m.recommendedSaving)}.` },
...
function executiveParagraphs(m: Metrics) {
  const global = Math.round(appScores(m).reduce((sum, item) => sum + item.score, 0) / 6);
  return [
    ...
    `Jubilación. La pensión estimada asciende a ${moneyMonth(m.retirementPension)}. Una vez consideradas las rentas inmobiliarias declaradas, la brecha mensual ajustada es de ${moneyMonth(m.retirementGap)} y el capital objetivo para financiar el retiro hasta los 90 años es de ${money(m.targetCapital)}.`,
    `Patrimonio proyectado. La proyección a los 67 años asciende a ${money(m.projectedTotal)}, desglosada entre dinero acumulado, dinero invertido, ahorro sistemático, rentas inmobiliarias e inversiones inmobiliarias. Si esta cifra queda por debajo del capital objetivo, conviene elevar aportaciones, revisar rentabilidades y separar liquidez, inversión y jubilación.`,
...
async function generatePdf() {
  window.dispatchEvent(new CustomEvent('audit-real-estate-updated')); await new Promise<void>((resolve) => window.setTimeout(resolve, 160));
  const clientName = fieldValue('nombre') || 'Cliente'; const m = collectMetrics(); const projects = collectProjectRows(m.capacity); const questions = collectQuestionRows(); const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' }); const state: PageState = { page: 1, y: 42, title: '' };
  ...
  newPage(doc, state, 'Auditoría de previsión social'); heading(doc, state, '2. Auditoría de Previsión Social'); paragraph(doc, state, 'Esta sección cuantifica el impacto de cada contingencia sobre la estabilidad económica del hogar. El objetivo es identificar qué prestaciones públicas cubren el gasto mensual y cuáles dejan una brecha que debe asegurarse o planificarse.'); rows(doc, state, m.benefits.map((b) => ({ label: b.label, value: `Prestación estimada: ${moneyMonth(b.value)}`, note: b.gap > 0 ? `Brecha: ${moneyMonth(b.gap)}` : 'Cubierto' })), 1); drawBenefitChart(doc, state, m); footer(doc, state.page++);
